...
// Use a common base URL for all API calls
// const String _baseUrl = 'http://192.168.0.12:5000/api'; // Example local IP
// const String _baseUrl = 'http://10.0.2.2:5000/api'; // For Android Emulator
const String _baseUrl =
    'https://englishbhashi-intenrship-task.onrender.com/api'; // Deployed backend
...
class ApiService {
  ...
  Future<Map<String, dynamic>> login(String email, String password) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
          'ApiService: Login failed with status ${response.statusCode}, body: ${response.body}',
        );
        throw Exception(
  ...
  Future<User> getProfile() async {
    final profileUri = Uri.parse('$baseUrl/auth/profile');
    print('ApiService: Attempting to GET $profileUri');
    try {
      final response = await http
          .get(profileUri, headers: await _getHeaders())
          .timeout(const Duration(seconds: 10));
      ...
      if (response.statusCode == 200) {
        ...
        print(
          'ApiService: Get profile failed with status ${response.statusCode}, body: ${response.body}',
        );
        throw Exception(
      ...
      throw Exception(
        'Get profile request timed out. Please check your connection.',
      );
    } catch (e) {
  ...
  Future<Course> createCourse(Map<String, dynamic> courseData) async {
    final createCourseUri = Uri.parse('$baseUrl/courses');
    print(
      'ApiService: Attempting to POST to $createCourseUri with data: $courseData',
    );
    try {
      final response = await http
          .post(
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        // 201 for created, 200 if it returns the course
        return Course.fromJson(jsonDecode(response.body));
      } else {
        print(
          'ApiService: Create course failed with status ${response.statusCode}, body: ${response.body}',
        );
        throw Exception(
          'Failed to create course: ${response.statusCode} ${response.reasonPhrase}',
        );
      }
    } on TimeoutException catch (e) {
      print('ApiService: Create course request timed out: $e');
      throw Exception(
        'Create course request timed out. Please check your connection.',
      );
    } catch (e) {
  ...
  Future<List<Course>> getCourses() async {
    final coursesUri = Uri.parse('$baseUrl/courses');
    print('ApiService: Attempting to GET $coursesUri');
    try {
      final response = await http
          .get(coursesUri, headers: await _getHeaders())
          .timeout(const Duration(seconds: 15));
      ...
      if (response.statusCode == 200) {
        ...
        if (decodedBody is Map<String, dynamic> &&
          ...
          if (coursesData is List) {
            ...
            print(
              'ApiService: Get courses expected a list for "courses" key, but got: ${coursesData.runtimeType}',
            );
            throw Exception(
              'Failed to parse courses: "courses" key did not contain a list.',
            );
          }
          ...
          print(
            'ApiService: Get courses received unexpected JSON format: ${response.body}',
          );
          throw Exception(
            'Failed to parse courses: Unexpected JSON format. Expected a List or a Map with a "courses" key.',
          );
        }
        ...
        return courseListJson.map((json) {
          if (json is Map<String, dynamic>) {
            ...
            print(
              'ApiService: Get courses found an item in the list that is not a Map: $json',
            );
            throw Exception(
              'Failed to parse courses: Invalid item in course list.',
            );
          }
        }).toList();
      } else {
        print(
          'ApiService: Get courses failed with status ${response.statusCode}, body: ${response.body}',
        );
        throw Exception(
      ...
      throw Exception(
        'Get courses request timed out. Please check your connection.',
      );
    } catch (e) {
  ...
  Future<Course> getCourseById(String courseId) async {
    final courseUri = Uri.parse('$baseUrl/courses/$courseId');
    print('ApiService: Attempting to GET $courseUri');
    try {
      final response = await http
          .get(courseUri, headers: await _getHeaders())
          .timeout(const Duration(seconds: 10));
      ...
      if (response.statusCode == 200) {
        ...
        print(
          'ApiService: Get course by ID failed with status ${response.statusCode}, body: ${response.body}',
        );
        throw Exception(
          'Failed to load course details: ${response.statusCode} ${response.reasonPhrase}',
        );
      }
    } on TimeoutException catch (e) {
      print('ApiService: Get course by ID request timed out: $e');
      throw Exception(
        'Get course by ID request timed out. Please check your connection.',
      );
    } catch (e) {
  ...
  Future<void> enrollCourse(String courseId) async {
    final enrollUri = Uri.parse('$baseUrl/courses/$courseId/enroll');
    print('ApiService: Attempting to POST to $enrollUri');
    try {
      final response = await http
          .post(enrollUri, headers: await _getHeaders())
          .timeout(const Duration(seconds: 10));
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        print(
          'ApiService: Failed to enroll in course. Status Code: ${response.statusCode}, Body: ${response.body}',
        );
        throw Exception('Failed to enroll in course: ${response.body}');
      }
    } on TimeoutException catch (_) {
      print(
        'ApiService: Enroll course request timed out for course $courseId.',
      );
      throw Exception(
        'Enroll course request timed out. Please check your connection.',
      );
    } catch (e) {
  ...
  Future<List<UserProgress>> getUserProgress() async {
    final progressUri = Uri.parse('$baseUrl/progress');
    print('ApiService: Attempting to GET $progressUri');
    try {
      final response = await http
          .get(progressUri, headers: await _getHeaders())
          .timeout(const Duration(seconds: 15));
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          ...
          print(
            'ApiService: Expected a list from getUserProgress, got: ${response.body}',
          );
          throw Exception(
            'Failed to parse user progress: Unexpected response format',
          );
        }
      } else {
        print(
          'ApiService: Failed to get user progress. Status Code: ${response.statusCode}, Body: ${response.body}',
        );
      ...
      throw Exception(
        'Get user progress request timed out. Please check your connection.',
      );
    } catch (e) {
  ...
  Future<UserProgress> getCourseProgress(String courseId) async {
    final courseProgressUri = Uri.parse('$baseUrl/progress/$courseId');
    print('ApiService: Attempting to GET $courseProgressUri');
    try {
      final response = await http
          .get(courseProgressUri, headers: await _getHeaders())
          .timeout(const Duration(seconds: 10));
      ...
      if (response.statusCode == 200) {
        ...
        print(
          'ApiService: Failed to get course progress for $courseId. Status Code: ${response.statusCode}, Body: ${response.body}',
        );
        throw Exception(
          'Failed to load course progress for $courseId: ${response.body}',
        );
      }
    } on TimeoutException catch (_) {
      print('ApiService: Get course progress request timed out for $courseId.');
      throw Exception(
        'Get course progress request timed out. Please check your connection.',
      );
    } catch (e) {
  ...
  Future<List<SectionModel>> getSectionsForCourse(String courseId) async {
    final sectionsUri = Uri.parse('$baseUrl/courses/$courseId/sections');
    print('ApiService: Attempting to GET $sectionsUri');
    try {
      final response = await http
          .get(sectionsUri, headers: await _getHeaders())
          .timeout(const Duration(seconds: 10));

      if (response.statusCode == 200) {
        final List<dynamic> sectionsListJson = jsonDecode(response.body);
        return sectionsListJson
            .map((json) => SectionModel.fromJson(json))
            .toList();
      } else {
        print(
          'ApiService: Get sections for course failed with status ${response.statusCode}, body: ${response.body}',
        );
        throw Exception(
          'Failed to load sections: ${response.statusCode} ${response.reasonPhrase}',
        );
      }
    } on TimeoutException catch (e) {
      print('ApiService: Get sections for course request timed out: $e');
      throw Exception(
        'Get sections for course request timed out. Please check your connection.',
      );
    } catch (e) {
  ...
  Future<ChapterModel> getChapterDetails(String chapterId) async {
    final chapterUri = Uri.parse('$baseUrl/chapters/$chapterId');
    print('ApiService: Attempting to GET $chapterUri');
    try {
      final response = await http
          .get(chapterUri, headers: await _getHeaders())
          .timeout(const Duration(seconds: 10));
      ...
      if (response.statusCode == 200) {
        ...
        print(
          'ApiService: Get chapter details failed with status ${response.statusCode}, body: ${response.body}',
        );
        throw Exception(
          'Failed to load chapter details: ${response.statusCode} ${response.reasonPhrase}',
        );
      }
    } on TimeoutException catch (e) {
      print('ApiService: Get chapter details request timed out: $e');
      throw Exception(
        'Get chapter details request timed out. Please check your connection.',
      );
    } catch (e) {
  ...
  Future<UserProgress> markChapterAsComplete(String chapterId) async {
    final completeChapterUri = Uri.parse('$baseUrl/progress/complete-chapter');
    print(
      'ApiService: Attempting to POST to $completeChapterUri for chapter $chapterId',
    );
    try {
      final response = await http
          .post(
      ...
      if (response.statusCode == 200) {
        ...
        print(
          'ApiService: Mark chapter complete failed with status ${response.statusCode}, body: ${response.body}',
        );
        throw Exception(
          'Failed to mark chapter as complete: ${response.statusCode} ${response.reasonPhrase}',
        );
      }
    } on TimeoutException catch (e) {
      print('ApiService: Mark chapter complete request timed out: $e');
      throw Exception(
        'Mark chapter complete request timed out. Please check your connection.',
      );
    } catch (e) {
      print('ApiService: Mark chapter complete error: $e');
      throw Exception(
        'Failed to mark chapter as complete. Please try again. ($e)',
      );
    }
  }
  ...
  Future<void> updateCurrentChapter(String courseId, String chapterId) async {
    final updateChapterUri = Uri.parse(
      '$baseUrl/progress/current-chapter/$courseId',
    );
    print(
      'ApiService: Attempting to POST to $updateChapterUri with chapter $chapterId',
    );
    try {
      final response = await http
          .post(
      ...
      if (response.statusCode == 200) {
        ...
        print(
          'ApiService: Update current chapter failed with status ${response.statusCode}, body: ${response.body}',
        );
        throw Exception(
          'Failed to update current chapter: ${response.statusCode} ${response.reasonPhrase}',
        );
      }
    } on TimeoutException catch (e) {
      print('ApiService: Update current chapter request timed out: $e');
      throw Exception(
        'Update current chapter request timed out. Please check your connection.',
      );
    } catch (e) {
      print('ApiService: Update current chapter error: $e');
      throw Exception(
        'Failed to update current chapter. Please try again. ($e)',
      );
    }
  }
  ...
  Future<UserProgress?> getUserProgressForCourse(String courseId) async {
    try {
      ...
      final response = await http
          .get(
            Uri.parse('$baseUrl/api/user/progress/$courseId'),
            headers: {
  ...
  Future<List<Course>> getEnrolledCourses() async {
    try {
      ...
      final response = await http
          .get(
            Uri.parse('$baseUrl/api/user/enrolled-courses'),
            headers: {
              'Content-Type': 'application/json',
              'Authorization': 'Bearer $token',
            },
      ...
      print(
        'ApiService: Get enrolled courses response: ${response.statusCode}',
      );
