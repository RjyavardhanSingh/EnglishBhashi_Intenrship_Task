...
export default function CourseViewPage() {
  ...
  const loadCourseData = useCallback(async () => {
    ...
    if (loadedRef.current && course && !loadingRef.current) {
      console.log('Course already loaded, skipping');
      return;
    }
      ...
      try {
        ...
        if (Array.isArray(progressResponse.data)) {
          courseProgress = progressResponse.data.find(p => 
            p.courseId === courseId || p.course?._id === courseId
          );
        } else if (progressResponse.data?.courseId === courseId) {
          courseProgress = progressResponse.data;
        }
        
        if (courseProgress) {
          console.log('Setting progress for enrolled user');
          
          const normalizedProgress: any = {
            courseId: courseId,
            ...
            sectionsProgress: courseProgress.sectionsProgress || []
          };
  ...
  }, [user, authLoading, courseId, loadCourseData]); // Stable dependencies only
  ...
  // Handle chapter selection with stable dependencies
  ...
  }, [progress, courseId, isCurrentChapterCompleted]); // Stable dependencies
  ...
  }, [currentChapter, forceRefreshProgress, isCurrentChapterCompleted]); // Stable dependencies
  ...
  const handleTextScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    // Consider 90% scrolled as "read to the end"
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-gray-300">Authenticating...</p>
        </div>
      </div>
    );
  }
  ...
  if (loading && !course) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600 mx-auto mb-4"></div>
          <p className="text-slate-600 dark:text-gray-300">Loading course...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
            <div className="px-6 py-8">
              <div className="text-center">
                ...
                <p className="text-gray-600 dark:text-gray-300 mb-8 max-w-2xl mx-auto">{course.description}</p>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-indigo-600">{sections.length}</div>
                    <div className="text-gray-600 dark:text-gray-300">Sections</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-indigo-600">
                      {sections.reduce((total, section) => total + (section.units?.length || 0), 0)}
                    </div>
                    <div className="text-gray-600 dark:text-gray-300">Units</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-indigo-600">
                    ...
                    <div className="text-gray-600 dark:text-gray-300">Chapters</div>
                  </div>
                </div>
            ...
            {progress && (
              <div className="mt-4">
                <div className="flex justify-between text-sm text-gray-600 dark:text-gray-300 mb-2">
                  <span>Progress</span>
                  <span>{progress.completed ? '100%' : `${Math.round(progress.overallProgress)}%`}</span>
                </div>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div 
                    className="bg-indigo-600 h-2 rounded-full transition-all duration-300"
                    style={{ 
                      width: progress.completed ? '100%' : `${Math.round(progress.overallProgress)}%` 
                    }}
                  ></div>
                </div>
              </div>
